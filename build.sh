...
setup_build_env() {
    ...
    # Clean or create BringBack directory
    if [ -d "$LIGHT_DIR" ]; then
        rm -rf "$LIGHT_DIR"/*
    else
        mkdir -p "$LIGHT_DIR"
...
setup_localversion() {
    ...
    local localversion="-BringBack-${MODEL}-${BUILD_KERNEL_VERSION}-A${PLATFORM_VERSION}"
...
compile_kernel() {
    ...
    if ! make ARCH=arm64 -j$(nproc --all) O=${OUT_DIR} exynos9820-${DEVICE}_defconfig bringback.config ksu.config; then
...
create_boot_img() {
    ...
    mv "${AIK_DIR}/image-new.img" "${LIGHT_DIR}/boot.img"
...
create_dtbo_img() {
    ...
    mv "dtbo.img" "${LIGHT_DIR}/dtbo.img"
...
create_odin_package() {
    ...
    if [ ! -f "boot.img" ] || [ ! -f "dt.img" ] || [ ! -f "dtbo.img" ]; then
        log_error "Required image files not found in ${LIGHT_DIR}"
        exit 1
    fi
    
    if ! tar -cvf "BringBack_${MODEL}_v.${BUILD_KERNEL_VERSION}_Odin_SUSFS.tar" boot.img dt.img dtbo.img; then
...
create_twrp_package() {
    ...
    if [ -d "$(pwd)/early_setting/META-INF" ]; then
        cp -ar "$(pwd)/early_setting/META-INF" "${LIGHT_DIR}/META-INF"
    else
    ...
    if ! zip -r "BringBack_${MODEL}_v.${BUILD_KERNEL_VERSION}_SUSFS_KSUN.zip" META-INF boot.img dt.img dtbo.img; then
...
main() {
    ...
    echo "📦 Output files in ${LIGHT_DIR}/:"
    echo "   - BringBack_${MODEL}_v.${BUILD_KERNEL_VERSION}_Odin_KSUN.tar"
    echo "   - BringBack_${MODEL}_v.${BUILD_KERNEL_VERSION}_TWRP_KSUN.zip"
    echo ""
